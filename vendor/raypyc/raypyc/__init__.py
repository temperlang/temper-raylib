...
_raylib_dynamic_library = ctypes.cdll.LoadLibrary(
    str(pathlib.Path(__file__).parent / DYNAMIC_LIBRARIES_PATH / dllname)
)
...
def evaluate_c_type_string_to_python_type(c_type_string):
    ...
    if is_array and pointer_level > 0:  # array of pointers to structures
        ...
        if (
            type_of_array_without_pointers_string in raypyc.structures.__structs
        ):  # a struct array pointer level 1+ or just a pointer level 1
            type_of_array_end = raypyc.structures.__structs[
                type_of_array_without_pointers_string
            ]
            for i in range(pointer_level):
                type_of_array_end = ctypes.POINTER(type_of_array_end)
        ...
        if (
            type_without_pointers_string in c_string_to_ctypes_string
        ):  # basic type pointer(probably double+ pointer level) (int**, char**, float**, ...)
            type_of_pointer_end = c_string_to_ctypes_string[
                type_without_pointers_string
            ]
            for i in range(pointer_level):
        ...
        if (
            type_without_pointers_string in raypyc.structures.__structs
        ):  # a struct pointer level 1+ or just a pointer level 1
            type_of_pointer_end = raypyc.structures.__structs[
                type_without_pointers_string
            ]
            for i in range(pointer_level):
                type_of_pointer_end = ctypes.POINTER(type_of_pointer_end)
    ...
    for function in functions_set:
        ...
        if do_wrapper_this_function:
            ...
            if function_returnType_processed.count("[") > 0:
                function_returnType_processed = function_returnType_processed.split(
                    "["
                )[0]
...
def wrap_functions_to_ctypes_functions_add_function_to_this_module(
    functions_to_wrap, _current_module
):
    for function_to_wrap in functions_to_wrap:
        if function_to_wrap["name"] not in wrapped_functions_names:
            ...
            name_of_function = (
                underscore(function_to_wrap["name"])
                .replace("3_d", "_3d")
                .replace("2_d", "_2d")
            )
            function_to_wrap_ctype = {
                "name": function_to_wrap["name"],
                "parametersTypes": [],
                "returnType": None,
            }
            ...
            if "params" in function_to_wrap.keys():
                for function_param in function_to_wrap["params"]:
                    if function_param["type"] != "...":
                        function_to_wrap_ctype["parametersTypes"].append(
                            evaluate_c_type_string_to_python_type(
                                function_param["type"]
                            )
                        )
            function_to_wrap_ctype["returnType"] = (
                evaluate_c_type_string_to_python_type(function_to_wrap["returnType"])
            )

            f = wrap_function(
                function_to_wrap_ctype["name"],
                function_to_wrap_ctype["parametersTypes"],
                function_to_wrap_ctype["returnType"],
            )
...
if sys.platform != "emscripten":
    raygui_functions_to_wrapped = check_for_functions_that_can_wrap(
        raygui_api_functions
    )

wrap_functions_to_ctypes_functions_add_function_to_this_module(
    rlgl_functions_to_wrapped, current_module
)
wrap_functions_to_ctypes_functions_add_function_to_this_module(
    raylib_functions_to_wrapped, current_module
)
wrap_functions_to_ctypes_functions_add_function_to_this_module(
    raymath_functions_to_wrapped, current_module
)
if sys.platform != "emscripten":
    wrap_functions_to_ctypes_functions_add_function_to_this_module(
        raygui_functions_to_wrapped, current_module
    )
