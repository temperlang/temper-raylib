...
_raypyc_extra_functions = ctypes.cdll.LoadLibrary(
    str(DYNAMIC_LIBRARIES_PATH / _raylib_dynamic_library_name)
)
...
def convert_c_type_to_python_type(c_type_string: str) -> str:
    ...
    c_type_string = c_type_string.replace(" ", "").replace(
        "const", ""
    )  # remove spaces, and "const" because there isn't really a const type in python...
    ...
    if (
        c_type_string in c_string_to_ctypes_string
    ):  # "regular" value not a pointer or an array
    ...
    if is_array and pointer_level > 0:  # array of pointers to structures
        ...
        if (
            type_of_array in c_string_to_ctypes_string
        ):  # basic type pointer (int*, char*, float*, ...)
            return f"ctypes.{c_string_to_ctypes_string[type_of_array]} * {array_size}"
        if (
            type_of_array_without_pointers in c_string_to_ctypes_string
        ):  # basic type pointer(probably double+ pointer level) (int**, char**, float**, ...)
            type_of_array_end = c_string_to_ctypes_string[
                type_of_array_without_pointers
            ]
        ...
        if (
            type_of_array in c_string_to_ctypes_string
        ):  # basic type array (int, char, float, ...)
            return f"ctypes.{c_string_to_ctypes_string[type_of_array]} * {array_size}"
        else:  # a struct array
        ...
        if (
            type_without_pointers in c_string_to_ctypes_string
        ):  # basic type pointer(probably double+ pointer level) (int**, char**, float**, ...)
            type_of_pointer_end = (
                "ctypes." + c_string_to_ctypes_string[type_without_pointers]
            )
            for i in range(pointer_level):
...
def generate_color_code(
    define_data: Dict[str, Union[str, float, int]], for_stub: bool = False
) -> str:
    if define_data["type"] == "COLOR":
        ints_array_string = (
            define_data["value"].split("{")[1].split("}")[0].replace(" ", "").split(",")
        )
...
def generate_dummy_struct_code(
    struct_name: str, bytes_size: int, for_stub: bool = False
) -> str:
...
def generate_struct_code(
    struct_data: Dict[str, Union[str, List[Dict[str, str]]]], for_stub: bool = False
) -> str:
    _string = ""
    struct_fields = ""
...
def generate_function_signature_code(
    function_data: Dict[str, Union[str, List[Dict[str, str]]]]
) -> str:
    ...
    if "params" in function_data.keys():  # only return stuff
        for param in function_data["params"]:
            function_string += (
                f"{param['name']}: {convert_c_type_to_python_type(param['type'])}, "
            )
...
def generate_dummy_structs_code(names: List[str], for_stub: bool = False) -> str:
    ...
    for name in names:
        ...
        dummy_struct_code = generate_dummy_struct_code(
            name, int(function_of_size()), for_stub
        )
...
def generate_structs_aliases_code(
    structs_api: List[Union[Dict[str, Union[str, List[Dict[str, str]]]], Any]],
    aliases_api: List[Union[Dict[str, str], Any]],
    for_stub: bool = False,
) -> str:
    _string = ""
    if not for_stub:
            ...
            for alias in aliases_api:
                if struct["name"] == alias["type"]:
                    if alias["name"] not in _wrapped_structures_names:
                        alias_string_logic = ""
                        alias_string_logic += generate_alias_code(
                            alias["name"], struct["name"]
                        )
...
def generate_structures_dictionary_code(
    wrapped_structures: List[str], for_stub: bool = False
) -> str:
    if not for_stub:
...
def check_for_functions_that_can_wrap(
    functions_api: List[
        Union[Any, Dict[str, Union[str, List[Dict[str, str]]]], Dict[str, str]]
    ]
) -> List[Union[Any, Dict[str, Union[str, List[Dict[str, str]]]], Dict[str, str]]]:
    ...
    for function in functions_api:
        ...
        if "params" in function.keys():
            for function_param in function["params"]:
                if is_string_contained_in_list(
                    function_param["type"],
                    [
                        "Sound",
                        "AudioCallback",
    ...
    for function in functions_set:
        if function["name"] not in wrapped_functions_names_stub:
            ...
            name_of_function = (
                ...
                .replace("3_d", "_3d")
                .replace("2_d", "_2d")
                .replace("vector_2", "vector2_")
                .replace("vector_3", "vector3_")
            )
...
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    "import ctypes\nfrom typing import Type\n\n",
)

# generate dummy structures code and add dummy structures code to files
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_dummy_structs_code(["rAudioBuffer", "rAudioProcessor"], for_stub=False),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_dummy_structs_code(["rAudioBuffer", "rAudioProcessor"], for_stub=True),
)

# generate structures code add structures code to files
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_structs_aliases_code(
        config_api_structs, config_api_aliases, for_stub=False
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_structs_aliases_code(rlgl_api_structs, rlgl_api_aliases, for_stub=False),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_structs_aliases_code(
        raylib_api_structs, raylib_api_aliases, for_stub=False
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_structs_aliases_code(
        raymath_api_structs, raymath_api_aliases, for_stub=False
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_structs_aliases_code(
        raygui_api_structs, raygui_api_aliases, for_stub=False
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_structs_aliases_code(
        config_api_structs, config_api_aliases, for_stub=True
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_structs_aliases_code(rlgl_api_structs, rlgl_api_aliases, for_stub=True),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_structs_aliases_code(
        raylib_api_structs, raylib_api_aliases, for_stub=True
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_structs_aliases_code(
        raymath_api_structs, raymath_api_aliases, for_stub=True
    ),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_structs_aliases_code(
        raygui_api_structs, raygui_api_aliases, for_stub=True
    ),
)
...
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.py",
    generate_structures_dictionary_code(wrapped_structures_names, for_stub=False),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "structures/__init__.pyi",
    generate_structures_dictionary_code(wrapped_structures_names_stub, for_stub=True),
)
# -----------------------------------------

generate_file(RAYPYC_FOLDER_PATH / "__init__.pyi")
add_text_to_file(
    RAYPYC_FOLDER_PATH / "__init__.pyi",
...
add_text_to_file(
    RAYPYC_FOLDER_PATH / "__init__.pyi",
    generate_functions_code(config_functions_to_wrapped),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "__init__.pyi",
    generate_functions_code(rlgl_functions_to_wrapped),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "__init__.pyi",
    generate_functions_code(raylib_functions_to_wrapped),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "__init__.pyi",
    generate_functions_code(raymath_functions_to_wrapped),
)
add_text_to_file(
    RAYPYC_FOLDER_PATH / "__init__.pyi",
    generate_functions_code(raygui_functions_to_wrapped),
)
